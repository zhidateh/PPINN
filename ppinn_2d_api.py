    ...
    def train(self):        
        ...
        for nSampling in range(self.num_sampling):
            ...
            for nEpoch in range(self.num_epoch):
                ...
                rand_SSE    = np.random.choice(range(x_SSE_sample.shape[0]),x_SSE_sample.shape[0],replace=False)
                rand_PINN   = np.random.choice(range(x_PINN_sample.shape[0]),x_PINN_sample.shape[0],replace=False)

                batch_size_SSE = int(x_SSE_sample.shape[0] * self.batch_ratio)
                batch_size_PINN = int(x_PINN_sample.shape[0] * self.batch_ratio)
                
                for nIt in range( int(1.0/self.batch_ratio)):

                    tf_dict = { self.P_back_SSE_tf  : P_back_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.x_SSE_tf       : x_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.y_SSE_tf       : y_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.P_SSE_tf       : P_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.rho_SSE_tf     : rho_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.u_SSE_tf       : u_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.v_SSE_tf       : v_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.Et_SSE_tf      : Et_SSE_sample[rand_SSE][nIt*batch_size_SSE: (nIt+1)*batch_size_SSE,:],
                                self.P_back_PINN_tf : P_back_PINN_sample[rand_PINN][nIt*batch_size_PINN: (nIt+1)*batch_size_PINN,:],
                                self.x_PINN_tf      : x_PINN_sample[rand_PINN][nIt*batch_size_PINN: (nIt+1)*batch_size_PINN,:],
                                self.y_PINN_tf      : y_PINN_sample[rand_PINN][nIt*batch_size_PINN: (nIt+1)*batch_size_PINN,:]}
                    ...
                    if nEpoch % 100 == 0:
                        ...
                        print("Sampling: %d, Epoch: %d, It, %d, Elapsed: %.2f min" %(nSampling, nEpoch, nIt, elapsed))
    ...
    def generateSample(self):
        ...
        if self.sampling_method == "case sampling":
            ...
            for case in range(len(cases)):
                ...
                batch_logic             = np.logical_and( self.x[index] >= 0, self.x[index] <= 0.42 ) #converging region of domain
                empty_batch_logic       = np.logical_and( self.x[index] >= 0, self.x[index] <= 0.84 ) #entire domain

                true_batch_per_case  = batch_per_case + batch_empty_per_case

                #check if the logic 1 result in empty array
                if (self.P[index][batch_logic].shape[0] != 0):
                    if batch_per_case  <= self.P[index][batch_logic].shape[0]:
                        A = np.random.choice(self.P[index][batch_logic].shape[0], size=batch_per_case , replace=False)
                    else:
                        A = np.random.choice(self.P[index][batch_logic].shape[0], size=batch_per_case , replace=True)
                    
                    if(len(A)>0):
                        P_back_SSE_batch    = np.hstack((P_back_SSE_batch, self.P_back[index][batch_logic][A]))
                        x_SSE_batch         = np.hstack((x_SSE_batch, self.x[index][batch_logic][A])) 
                        y_SSE_batch         = np.hstack((y_SSE_batch, self.y[index][batch_logic][A]))
                        P_SSE_batch         = np.hstack((P_SSE_batch, self.P[index][batch_logic][A]))
                        rho_SSE_batch       = np.hstack((rho_SSE_batch, self.rho[index][batch_logic][A]))
                        u_SSE_batch         = np.hstack((u_SSE_batch, self.u[index][batch_logic][A]))
                        v_SSE_batch         = np.hstack((v_SSE_batch, self.v[index][batch_logic][A]))
                        Et_SSE_batch        = np.hstack((Et_SSE_batch, self.Et[index][batch_logic][A]))

                        P_back_PINN_batch    = np.hstack((P_back_PINN_batch, self.P_back[index][batch_logic][A]))
                        x_PINN_batch         = np.hstack((x_PINN_batch, self.x[index][batch_logic][A])) 
                        y_PINN_batch         = np.hstack((y_PINN_batch, self.y[index][batch_logic][A]))
                ...
                if (self.P[index][empty_batch_logic].shape[0] != 0):

                    if  batch_empty_per_case  <= self.P[index][empty_batch_logic].shape[0]:
                        B = np.random.choice(self.P[index][empty_batch_logic].shape[0], size=batch_empty_per_case , replace=False)
                    else:
                        B = np.random.choice(self.P[index][empty_batch_logic].shape[0], size=batch_empty_per_case , replace=True)

                    if(len(B)>0):
                        P_back_PINN_batch    = np.hstack((P_back_PINN_batch, self.P_back[index][empty_batch_logic][B]))
                        x_PINN_batch         = np.hstack((x_PINN_batch, self.x[index][empty_batch_logic][B])) 
                        y_PINN_batch         = np.hstack((y_PINN_batch, self.y[index][empty_batch_logic][B]))
            ...
            wall_logic = 0
            w = np.random.choice(self.P_w[:,self.x_w[1]>= wall_logic].shape[1], size= int(math.ceil(self.bc_ratio*self.x_w.shape[1])) , replace=False)  #10% x 211
            ...
            P_back_SSE_batch    = np.hstack((P_back_SSE_batch,self.P_back_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten(),self.P_back_i[cases][:,i].flatten() ,self.P_back_o[cases][:,o].flatten()   ,self.P_back_c[cases][:,c].flatten())).reshape(-1,1) 
            x_SSE_batch         = np.hstack((x_SSE_batch     ,self.x_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten()     ,self.x_i[cases][:,i].flatten()      ,self.x_o[cases][:,o].flatten()        ,self.x_c[cases][:,c].flatten())).reshape(-1,1)
            y_SSE_batch         = np.hstack((y_SSE_batch     ,self.y_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten()     ,self.y_i[cases][:,i].flatten()      ,self.y_o[cases][:,o].flatten()        ,self.y_c[cases][:,c].flatten())).reshape(-1,1)
            P_SSE_batch         = np.hstack((P_SSE_batch     ,self.P_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten()     ,self.P_i[cases][:,i].flatten()      ,self.P_o[cases][:,o].flatten()        ,self.P_c[cases][:,c].flatten())).reshape(-1,1)
            rho_SSE_batch       = np.hstack((rho_SSE_batch   ,self.rho_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten()   ,self.rho_i[cases][:,i].flatten()    ,self.rho_o[cases][:,o].flatten()      ,self.rho_c[cases][:,c].flatten())).reshape(-1,1)
            u_SSE_batch         = np.hstack((u_SSE_batch     ,self.u_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten()     ,self.u_i[cases][:,i].flatten()      ,self.u_o[cases][:,o].flatten()        ,self.u_c[cases][:,c].flatten())).reshape(-1,1)
            v_SSE_batch         = np.hstack((v_SSE_batch     ,self.v_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten()     ,self.v_i[cases][:,i].flatten()      ,self.v_o[cases][:,o].flatten()        ,self.v_c[cases][:,c].flatten())).reshape(-1,1)
            Et_SSE_batch        = np.hstack((Et_SSE_batch    ,self.Et_w[cases][:,self.x_w[1]>= wall_logic][:,w].flatten()    ,self.Et_i[cases][:,i].flatten()     ,self.Et_o[cases][:,o].flatten()       ,self.Et_c[cases][:,c].flatten())).reshape(-1,1)
            ...
            if P_back_PINN_batch.shape[0] != 0:
                ...
                P_back_PINN_batch    = np.vstack((P_back_PINN_batch ,self.P_back_w[cases][:,self.x_w[1]>= wall_logic][:,w].transpose()  ,self.P_back_i[cases][:,i].transpose()  ,self.P_back_o[cases][:,o].transpose()))
                x_PINN_batch         = np.vstack((x_PINN_batch      ,self.x_w[cases][:,self.x_w[1]>= wall_logic][:,w].transpose()       ,self.x_i[cases][:,i].transpose()       ,self.x_o[cases][:,o].transpose()))
                y_PINN_batch         = np.vstack((y_PINN_batch      ,self.y_w[cases][:,self.x_w[1]>= wall_logic][:,w].transpose()       ,self.y_i[cases][:,i].transpose()       ,self.y_o[cases][:,o].transpose()))
                ...
                P_back_PINN_batch    = np.vstack((self.P_back_w[cases][:,self.x_w[1]>= wall_logic][:,w].transpose()  ,self.P_back_i[cases][:,i].transpose()  ,self.P_back_o[cases][:,o].transpose()))
                x_PINN_batch         = np.vstack((self.x_w[cases][:,self.x_w[1]>= wall_logic][:,w].transpose()       ,self.x_i[cases][:,i].transpose()       ,self.x_o[cases][:,o].transpose()))
                y_PINN_batch         = np.vstack((self.y_w[cases][:,self.x_w[1]>= wall_logic][:,w].transpose()       ,self.y_i[cases][:,i].transpose()       ,self.y_o[cases][:,o].transpose()))
